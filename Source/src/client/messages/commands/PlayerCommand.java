...
public class PlayerCommand {
    ...
    public static class ea extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "經驗值倍率 " + (Math.round(c.getPlayer().getEXPMod()) * 100) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + "%, 掉寶倍率 " + Math.round(c.getPlayer().getDropMod() * (c.getPlayer().getStat().dropBuff / 100.0) * 100) + "%, 楓幣倍率 " + Math.round((c.getPlayer().getStat().mesoBuff / 100.0) * 100) + "%");
