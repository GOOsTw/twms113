...
function action(mode, type, selection) {
    if (status >= 27 && mode == 0) {
	cm.sendNext("還有很多地方可以參觀呢！, 讓我知道你想去哪裡玩吧！？.");
	cm.dispose();
	return;
	...
	status--;
    if (status == 0) {
	cm.sendNext("你想要到其他的城市?雖然需要的錢不多, 但對初心者有90%的折扣哦.");
    } else if (status == 1) {
	...
	if (selection == 0) {
	    cm.sendSimple("維多利亞有七個城鎮，你想多了解哪ㄧ個呢？\r\n#b#L0##m104000000##l\r\n#L1##m102000000##l\r\n#L2##m101000000##l\r\n#L3##m100000000##l\r\n#L4##m103000000##l\r\n#L5##m120000000##l\r\n#L6##m105040300##l");
	} else if (selection == 1) {
	    status = 26;
	    if (cm.getJob() == 0) {
		var selStr = "對於新手我們有打9折的優惠唷. 你想去哪呢?#b";
		for (var i = 0; i < maps.length; i++) {
		    selStr += "\r\n#L" + i + "##m" + maps[i] + "# (" + costBeginner[i] + " mesos)#l";
		}
	    } else {
		var selStr = "啊，你不是新手啊? 那我只能跟你收全部的錢了. 你想去哪裡?#b";
		for (var i = 0; i < maps.length; i++) {
